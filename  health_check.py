...
def check_memory(warn_pct: int = 75, crit_pct: int = 90) -> Tuple[str, str]:
    """
    Verifica el porcentaje de RAM utilizada en el sistema usando psutil.
    warn_pct: umbral de advertencia (%). crit_pct: umbral crítico (%).
    ...
    try:
        ...
        used_pct = memory.percent  # Porcentaje de RAM usada
        available_gb = memory.available / (1024 ** 3)  # Convertir bytes a GB

        if used_pct >= crit_pct:
            return 'error', f'Memoria crítica: {used_pct:.1f}% usado ({available_gb:.1f}GB disponible)'
        if used_pct >= warn_pct:
            return 'warning', f'Memoria alta: {used_pct:.1f}% usado ({available_gb:.1f}GB disponible)'
        return 'ok', f'Memoria saludable: {used_pct:.1f}% usado ({available_gb:.1f}GB disponible)'
    except ImportError:
